...
def register(client):
    ...
    async def kang_jutsu(event):
        ...
        if not reply.media: 
            return await event.edit("`No chakra detected.`")

        emoji = event.pattern_match.group(1) or "🤔"
        pack_name = event.pattern_match.group(2) or "main"
        await event.edit(f"`Sealing into {pack_name} vault...` 🌀")

        try:
            me = await client.get_me()
            # This name stays the same for YOU forever.
            ...
            async with client.conversation("@Stickers") as conv:
                # --- STEP 1: FORCE RESET ---
                # This stops any 'hanging' commands from previous tries
                ...
                if "Choose the sticker" in res.text:
                    # PACK EXISTS: Just adding the sticker
                    ...
                    await conv.get_response()
                    status_text = f"✅ **Sticker Added to {pack_name}!**"
                else:
                    # PACK DOES NOT EXIST: Create it ONCE
                    await event.edit("`Vault not found. Creating unique Scroll...` 📜")
                    ...
                    await conv.send_message(pack_short)
                    await conv.get_response()
                    status_text = f"🌟 **New Vault Created: {pack_name}!**"
    ...
    async def stext_jutsu(event):
        ...
        await event.edit("`Inking text...` ✍️")
        ...
        def create_text_sticker():
            ...
            for line in lines:
                ...
                draw.text(((512 - (right - left)) // 2, y), line, font=font, fill="white", stroke_width=3, stroke_fill="black")
                y += 55
